...
func newIndexCreateFlagSet() *pflag.FlagSet {
	...
	flagSet.Var(&indexCreateFlags.hnswMaxEdges, flags.HnswMaxEdges, "Maximum number bi-directional links per HNSW vertex. Greater values of 'm' in general provide better recall for data with high dimensionality, while lower values work well for data with lower dimensionality. The storage space required for the index increases proportionally with 'm'. Defaults to 16") //nolint:lll // For readability
	flagSet.Var(&indexCreateFlags.hnswConstructionEf, flags.HnswConstructionEf, "The number of candidate nearest neighbors shortlisted during index creation. Larger values provide better recall at the cost of longer index update times. Defaults to 100.")                                                                                                                    //nolint:lll // For readability
	flagSet.Var(&indexCreateFlags.hnswEf, flags.HnswEf, "The default number of candidate nearest neighbors shortlisted during search. Larger values provide better recall at the cost of longer search times. Defaults to 100.")                                                                                                                                                  //nolint:lll // For readability
	flagSet.Var(&indexCreateFlags.hnswMaxMemQueueSize, flags.HnswMaxMemQueueSize, "Maximum size of in-memory queue for inserted/updated vector records. Defaults to 1_000_000 records.")                                                                                                                                                                                          //nolint:lll // For readability                                                                                                                                                                                                                      //nolint:lll // For readability
